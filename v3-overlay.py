...
class PDFRenamerGUI:
    ...
    def setup_ui(self):
        ...
        overlay_btn = tk.Button(overlay_frame, text="Select Overlay PDF", command=self.browse_overlay)
    ...
    def browse_overlay(self):
        file = filedialog.askopenfilename(
            title="Select overlay PDF",
            filetypes=[("PDF files", "*.pdf"), ("All files", "*.*")]
        )
        if file:
            self.overlay_path.set(file)
            self.log("Selected overlay PDF: " + file)
    ...
    def apply_overlay(self, pdf_path, overlay_path):
        ...
        try:
            ...
            for page_num in range(len(background)):
                ...
                bg_page = background[page_num]
                # Create a new page in result document with same dimensions
                new_page = result_doc.new_page(width=bg_page.rect.width, height=bg_page.rect.height)
                # First, copy the background page content
                new_page.show_pdf_page(new_page.rect, background, page_num)
                # Then overlay the first page of the overlay PDF
        ...
        try:
            ...
            for i, file in enumerate(pdf_files, 1):
                ...
                try:
                    ...
                    if not self.apply_overlay(dest, self.overlay_path.get()):
                        self.log(f"  ✗ Overlay failed, but continuing with rename...")
                    else:
                        self.log(f"  ✓ Overlay applied")
